...
const TicketAssess = () => {
  ...
  const [componentKey, setComponentKey] = useState(0); // Add a key state
  ...
  useEffect(() => {
    ...
    fetchTickets();
  }, [componentKey]); // Include componentKey in the dependency array
  ...
  return (
    <div key={componentKey}> {/* Use the key to trigger a re-render */}
      <AppNavbar />
                ...
                .map((ticket) => (
                  <div key={ticket.id}>
                    ...
                    <form className="mt-3">
                      ...
                      <button
                        type="button"
                        className={`btn ${ticketStatus === 'Closed' ? 'btn-secondary' : 'btn-danger'}`}
      ...
      {/* Place ToastContainer at a higher level */}
      <ToastContainer />
    </div>
  );
};
