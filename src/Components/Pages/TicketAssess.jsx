...
const TicketAssess = () => {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        const q = query(collection(db, 'tickets'), where('assignTo', '==', auth.currentUser?.email), where('status', 'in', ['New!', 'In Progress'])
        );
  ...
  useEffect(() => {
    ...
    // Set the initial ticket status when tickets are fetched
    if (tickets[pageNumber]) {
      setTicketStatus(tickets[pageNumber].status || '');
    }
  }, [tickets, pageNumber]);
  ...
  const handleCloseTicket = async () => {
    try {
      ...
      // Fetch tickets again to update the state
      const q = query(collection(db, 'tickets'), where('assignTo', '==', auth.currentUser?.email));
      const querySnapshot = await getDocs(q);
  ...
  const changePage = ({ selected }) => {
    ...
    setTicketStatus(tickets[selected]?.status || '');
  };
  ...
  return (
    <div key={componentKey}> 
      <AppNavbar />
      <Container className="mt-4 d-flex justify-content-center" style={{ maxHeight: '800px', maxWidth: '750px', overflowY: 'auto' }}>
        <div className="cards">
          {tickets.length > 0 && (
            <>
              {tickets
                    ...
                    <Card className="custom-card" style={{ width: '42rem' }}>
                      ...
                      <Card.Body>
                        <Card.Text>
                          ...
                          <br /> <br />
                          <strong>Author:</strong> {ticket.createdBy}
                        </Card.Text>
                      </Card.Body>
                    </Card>
                    <Container />
                    ...
                    <form className="mt-3">
                      <div className="form-group">
                        ...
                        <textarea
                          ...
                          disabled={ticketStatus === 'Closed'} // Disable if ticket status is "Closed"
                        ></textarea>
                      </div>
                    
                      <button type="button"className="btn btn-primary mr-2"onClick={handleSendAnswer} disabled={ticketStatus === 'Closed'} >Send Answer</button>
                      <button type="button"className={`btn ${ticketStatus === 'Closed' ? 'btn-secondary' : 'btn-danger'}`} onClick={handleCloseTicket} disabled={ticketStatus === 'Closed'}>
          ...
          {!loading && tickets.length === 0 && (
            <Card className="custom-card" style={{ width: '42rem' }}>
              <Card.Body>
                <Card.Text>No tickets assigned to you.</Card.Text>
              </Card.Body>
            </Card>
          )}
        </div>
